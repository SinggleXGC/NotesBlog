...
public class LoginController {
    ...
    public ResponseDTO login(@RequestParam String username, @RequestParam String password) {
        ...
        if (username.equals("") || password.equals("")) {
            return new ResponseDTO(STATUS.FORBIDDEN.getStatus(), "请输入账号或密码", null);
        }
        User user = userService.selectUserByUserName(username);
        if (user == null) {
            responseDTO = new ResponseDTO(STATUS.FORBIDDEN.getStatus(), "用户名不存在", null);
        }else if (!password.equals(user.getPassword())) {
            responseDTO = new ResponseDTO(STATUS.FORBIDDEN.getStatus(), "密码错误", null);
        }else {
            user.setPassword("");
            user.setAvatarUrl(globalProperties.getBaseurl() + user.getAvatarUrl());
            responseDTO = new ResponseDTO(STATUS.OK.getStatus(), "登录成功", user);
        }
        return responseDTO;
    }
}
